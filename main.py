...
class Calculator(QWidget):
    def __init__(self):
        ...
        self.hbox_result.addWidget(self.b_mult)

        self.b_demult = QPushButton("%", self)
        self.hbox_result.addWidget(self.b_demult)
        ...
        self.b_demult.clicked.connect(lambda: self._operation("%"))
        ...
        if _number(self.input.text()):
          if self.num_2 == 0:
            ...
            if self.op == "*":
                self.input.setText(str(self.num_1 * self.num_2))
            if self.op == "%":
                return self.input.setText("На 0 делить нельзя")
          else:
            if self.op == "%":
                self.input.setText(str(self.num_1 / self.num_2))
            if self.op == "+":
